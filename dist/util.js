...
function createContext(program) {
    ...
    const normalizePathForGlob = (value) => {
        if (!value) {
            return value;
        }
    ...
    const ignorePatterns = (ignores || []).map(pattern => {
        var _a;
        const normalizedPattern = (_a = normalizePathForGlob(pattern)) !== null && _a !== void 0 ? _a : '';
    ...
    return {
        ...
        ignores: (file) => {
            ...
            const candidatePaths = [
                normalizePathForGlob(file.pkgPath),
                normalizePathForGlob(file.srcPath),
                normalizePathForGlob(file.pathAbsolute)
            ].filter(Boolean);
            return candidatePaths.some(candidatePath => ignorePatterns.some(pattern => minimatch(candidatePath, pattern)));
        },
        fix,
        checkUsage,
        addFixes: (file, entry) => {
